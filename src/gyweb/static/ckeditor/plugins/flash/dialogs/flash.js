...
(function () {
    function b(a, b, c) {
        var h = n[this.id];
        if (h)for (var f = this instanceof CKEDITOR.ui.dialog.checkbox, e = 0; e < h.length; e++) {
            var d = h[e];
            switch (d.type) {
                case 1:
                    if (!a)continue;
    ...
    function c(a, b, c) {
        var h = n[this.id];
        if (h)for (var f = "" === this.getValue(), e = this instanceof CKEDITOR.ui.dialog.checkbox, d = 0; d < h.length; d++) {
            var g = h[d];
            switch (g.type) {
                case 1:
                ...
                case 2:
                    if (!a)continue;
                    m = this.getValue();
                    if (f || e && m === g["default"])g.name in c && c[g.name].remove(); else if (g.name in c)c[g.name].setAttribute("value", m); else {
    ...
    CKEDITOR.dialog.add("flash", function (a) {
        ...
        return {
            title: a.lang.flash.title, minWidth: 420, minHeight: 310, onShow: function () {
                ...
                e && d.appendTo(e)));
                if (e)for (var b = {}, c = e.getElementsByTag("param", "cke"), f = 0, h = c.count(); f < h; f++)b[c.getItem(f).getAttribute("name")] = c.getItem(f);
                c = {};
                f = {};
